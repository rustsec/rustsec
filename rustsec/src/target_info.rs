...
use crate::{error, package::Name, package::Package, Lockfile};
...
impl TargetPackageInfo {
    ...
    fn filter_name<'a>(&self, lockfile: &'a Lockfile) -> Result<Option<Vec<&'a Package>>> {
        let target_packages: Vec<&Package> = lockfile
            .packages
            .iter()
    ...
    fn filter_version(&self, targets: Vec<&Package>) -> Result<Option<Package>> {
        match self.version.clone() {
            ...
            None => Err(Error::new(
                ErrorKind::MultipleTargetsFound,
                &format!(
                    "{}\n{}",
                    self.cli_identifiers,
                    TargetPackageInfo::print_ids(targets)
                ),
            )),
            Some(version) => {
                ...
                match filtered_targets.len() {
                    0 => Err(Error::new(
                        ErrorKind::NoTargetFound,
                        &self.cli_identifiers.to_string(),
                    )),
        ...
        match self.url.clone() {
            ...
            Some(url) => {
                let filtered_targets: Vec<&Package> = targets
                    .into_iter()
                    .filter(|package| {
                        package.source.is_some() && *package.source.clone().unwrap().url() == url
                    })
                    .collect();
                ...
                match filtered_targets.len() {
                    ...
                    _ => Err(Error::new(
                        ...
                        &format!(
                            "{}\n{}",
                            self.cli_identifiers,
                            TargetPackageInfo::print_ids(filtered_targets)
                        ),
                    )),
                }
            }
