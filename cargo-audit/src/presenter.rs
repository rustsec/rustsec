...
impl Presenter {
    ...
    /// Prints the vulnerability report generated by an audit.
    /// Returns `true` if the process should exit with a failure, `false` otherwise.
    #[must_use]
    pub fn print_report(
        &mut self,
        ...
        if self.config.format == OutputFormat::Json {
            ...
            return false; // icky but left intact for backwards compatibility
        }
